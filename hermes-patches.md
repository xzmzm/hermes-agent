...
> 这些是对 `~/.hermes/hermes-agent/` 的本地修改，维护在 `local-patches-v2` 分支上。
>
> **工作流**：
> 1. patch 维护在 `local-patches-v2` 分支（每个 patch 独立 commit，便于回滚与 cherry-pick）
> 2. `hermes update` 更新 `main` 后，在新 main 上手动逐个 re-apply（squashed net-diff reapply 在 +106 commits 结构性变更下会静默丢 hunk，见下方历史）
> 3. Fork 备份：`git push fork local-patches-v2`（remote name: `fork`，即 `github-hermes:xzmzm/hermes-agent.git`）
> 4. 旧分支保留为 `local-patches` / `local-patches-archive`（完整历史，含废弃 commit）；每次更新前打 tag `local-patches-pre-update-<date>`
>
...
## Active Patches (15 个编号 patch on `local-patches-v2`, upstream `d6269da7f`)

Rebased on upstream `d6269da7f`（v2026.6.19，+106 commits from `60cc42e38`）。
2026-06-25 更新时，因 `auxiliary_client.py` +298/-298 重写 + `telegram.py → plugins/platforms/telegram/adapter.py` 重命名，squashed net-diff reapply 静默丢 hunk，改为在新 main 上逐 patch 手动 re-apply。
旧 `local-patches` 分支（squashed commit `cd7c8ae37`）完整保留在 tag `local-patches-pre-update-20260625` 和分支 `local-patches-archive`。
