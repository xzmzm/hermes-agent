...
@pytest.mark.asyncio
async def test_quota_command_explicit_model_resolves_provider_key(monkeypatch):
    from gateway.run import GatewayRunner
    ...
    event = MagicMock()
    event.get_command_args.return_value = "glm-5.1"
    event.source = MagicMock()
    ...
    monkeypatch.setattr(
        "hermes_cli.runtime_provider.resolve_runtime_provider",
        lambda: {"api_key": "codex-token", "provider": "openai-codex", "model": "gpt-5.5"},
    )
    monkeypatch.setattr("hermes_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
    monkeypatch.setenv("GLM_API_KEY", "a1a1a1")
    ...
    assert captured["api_key"] == "a1a1a1"
